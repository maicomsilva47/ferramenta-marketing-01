...
const DiagnosticQuestion: React.FC<DiagnosticQuestionProps> = ({
  ...
  useEffect(() => {
    ...
    // Iniciar o cooldown ao trocar a pergunta
    ...
    const loadTimer = setTimeout(() => {
      ...
      // Após o conteúdo carregar, aguardar mais 1 segundo antes de permitir interação
  ...
  const handleOptionSelect = (value: OptionValue) => {
    // Não permitir seleção durante cooldown ou processamento
    if (cooldownActive || isSubmitting || isProcessing) {
      console.log("Seleção bloqueada - em período de cooldown ou processando");
      return;
    }
  ...
  // Determinar se deve mostrar o conteúdo ou indicador de carregamento
  const showContent = contentLoaded;
  const showOptions = contentLoaded && !cooldownActive;
              ...
              <div className="space-y-3 md:space-y-4 max-h-[55vh] md:max-h-[60vh] overflow-y-auto pr-3 pb-4 pt-1 pl-1">
                {showOptions ? (
                  question.options.map((option, index) => {
                    ...
                    return (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        disabled={isSubmitting || isProcessing || cooldownActive}
                      >
                  ...
                  <div className="flex items-center justify-center py-8">
                    <p className="text-gray-500 text-sm">Aguarde um momento para selecionar uma opção...</p>
                  </div>
                )}
              </div>
